...
int phy_id ( void );
void phy_reset ( void );
void phy_autonegotiate ( int );
void phy_set_link_status ( int );
void phy_link_setup ( void );
...
{
	if ( phy_up )
	    kprintf ( "PHY link detected UP at " );
	else
	    kprintf ( "PHY link detected DOWN at " );
...
phy_autonegotiate ( int speed_10 )
{
	...
	if ( speed_10 ) {
	    kprintf ( "PHY - limit autoneg speed to 10\n" );
...
phy_reset ( void )
{
	...
	/* start reset, this bit self clears */
        ...
        // kprintf ( "PHY reset cleared in %d counts\n", (RESET_TIMEOUT-tmo) );
	if ( phy_read(PHY_BMCR) & BMCR_RESET )
	    kprintf ( "PHY reset fails\n" );
}
...
phy_spin ( void )
{
	...
	// kprintf ( "phy_spin count = %d\n", tmo );
	if ( ep->mii_cmd & MII_BUSY  )
	    kprintf ( "PHY spin timeout\n" );
}
...
phy_read ( int reg )
{
	...
	// kprintf ( "phy_read A called: reg, cmd, val = %d, %04x, %04x\n", reg, cmd, val1 );
	...
	// kprintf ( "phy_read B called: reg, cmd, val = %d, %04x, %04x\n", reg, cmd, val1 );
	...
	if ( val1 != val2 ) {
	    kprintf ( "Mismatch in phy_read()\n" );
	    kprintf ( "phy_read A: reg, cmd, val = %d, %04x, %04x\n", reg, cmd, val1 );
	    kprintf ( "phy_read B: reg, cmd, val = %d, %04x, %04x\n", reg, cmd, val2 );
	}
...
phy_write ( int reg, int val )
{
	...
	if ( phy_debug )
	    kprintf ( "phy_write called %d, %04x, %04x\n", reg, val, cmd );
...
by accident.  I rebuilt U-boot sprinkling kprintf statements all through
...
--- 	// struct emac *ep = EMAC_BASE;
--- 
--- 	kprintf ( "READ1 Values from U-Boot:\n" );
--- 	kprintf ( "BMCR  = %04x\n", phy_read ( PHY_BMCR ) );
--- 	kprintf ( "ID1  = %04x\n", phy_read1 ( PHY_ID1 ) );
--- 	kprintf ( "ID2  = %04x\n", phy_read1 ( PHY_ID2 ) );
--- 	kprintf ( "BMCR  = %04x\n", phy_read ( PHY_BMCR ) );
...
--- 
--- 	/*
--- 	kprintf ( "EARLY Values from U-Boot:\n" );
--- 	kprintf ( "BMCR  = %04x\n", phy_read ( PHY_BMCR ) );
--- 	kprintf ( "BMSR  = %04x\n", phy_read ( PHY_BMSR ) );
--- 	kprintf ( "ADV   = %04x\n", phy_read ( PHY_ADVERT ) );
--- 	kprintf ( "PEER  = %04x\n", phy_read ( PHY_PEER ) );
...
--- 
--- 	if ( ! link_good ) {
---             kprintf("link down (emac)\n");
--- 	    return;
--- 	}
--- 
--- 	kprintf ( "link up (emac), speed %d, %s duplex\n",
...
--- phy_show ( void )
--- {
---         kprintf ( "PHY status BMCR: %04x, BMSR: %04x\n",
...
---         int tmo = AN_TIMEOUT;
--- 
--- 	kprintf ( "Autonegotiation ...\n" );
--- 	phy_uboot ();
--- 
--- 	phy_write ( PHY_ADVERT, 0 );
--- 	kprintf ( "I set: ADV   = %04x\n", phy_read ( PHY_ADVERT ) );
--- 	phy_write ( PHY_ADVERT, 0 );
--- 	kprintf ( "I set: ADV   = %04x\n", phy_read ( PHY_ADVERT ) );
...
--- 	kprintf ( "Starting Autonegotiation:\n" );
--- 	kprintf ( "BMSR   = %04x\n", phy_read ( PHY_BMSR ) );
---         reg = phy_read ( PHY_BMCR );
...
--- 
--- 	if ( ! tmo ) {
--- 	    kprintf ( " *** *** Autonegotiation timeout\n" );
--- 	    phy_show ();
--- 	    link_good = 0;
--- 	    phy_set_link ();	/* XXX */
--- 	    return;
--- 	}
--- 
--- 	kprintf ( "Autonegotiation finished\n" );
--- 	kprintf ( "Aneg done in %d milliseconds\n", AN_TIMEOUT - tmo );
--- 	kprintf ( "ADV   = %04x\n", phy_read ( PHY_ADVERT ) );
--- 	kprintf ( "PEER  = %04x\n", phy_read ( PHY_PEER ) );
--- 
--- 	phy_show ();
...
--- 	phy_set_link ();
--- 
--- 	// kprintf ( "Finished with Phy Init\n" );
--- }
#endif /* ANCIENT_PHY */
