...
// 	}
	...
	//This check definitely works - when I try it with the ethernet unplugged, it reports link down, and it proceeds when the link is up otherwise.
	uint32 reg;
...
// 	tdescptr->next = NULL;
	...
	//TODO Check status bit in DMA descriptor to see if that indicates if it has been transmitted
	kprintf("desc status before: %d\n", tdescptr->status);

	kprintf("Getting to ethwrite. Next step in comment\n");
	//TODO Not sure if the buf we copy here is in the correct format for our tx ring. Here is the relevant section of tom's code for cross-referencing: https://github.com/trebisky/Kyu/blob/3d9f63cd990c48ac37c7841314fe072cec68f9e3/orange_pi/emac.c#L1392
	...
	//TODO Something is wrong here, because our transmit interrupt in ethhandler.c never gets triggered
...
// 		//According to pg 585 of h3 datasheet, it appears that the nth descriptor's status entry will point to the address of the first descriptor (If I am interpreting the diagram correctly)
// 		while(prev->next != NULL && prev->next != csrptr->tx_dma_desc_list) {
// 			prev = prev->next;
// 		}
// 		prev->next = tdescptr;
// 	}
